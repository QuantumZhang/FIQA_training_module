...
class FocalBCEloss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        if self.weight is not None:
            dimn = batch_loss.shape[0]
            weight = torch.zeros(dimn, 2)
            for i in range(int(dimn/6)):
                weight[i*6:(i+1)*6] = self.weight
            weight = weight.cuda()
            batch_loss = (weight * class_mask).sum(dim=1).view(-1, 1) * batch_loss
